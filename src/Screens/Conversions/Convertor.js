...
const Converter = () => {
  ...
  const name1 = () => {
    let itemName = dry
      ? itemsDryTranslation[itemsDry[firstValue]]
      : itemsWetTranslation[itemsWet[firstValue]];
    return itemName;
  };
  const name2 = () => {
    let itemName = dry
      ? itemsDryTranslation[itemsDry[secondValue]]
      : itemsWetTranslation[itemsWet[secondValue]];
    return itemName;
  };
  ...
  useEffect(() => {
    ...
    if (num1 === "") return;
    let value = dry
      ? convert(num1).from(itemsDry[firstValue]).to(itemsDry[secondValue])
      : convert(num1).from(itemsWet[firstValue]).to(itemsWet[secondValue]);
    // in case of int
    try {
    ...
    <View style={styles.convert}>
      ...
      <View style={styles.masterSwitch}>
        ...
        <Switch
          ...
          onValueChange={() => {
            setDry((c) => !c);
            value1((c) => defaultState[0]);
            value2((c) => defaultState[1]);
          }}
          value={dry}
          style={[styles.switch]}
        />
      ...
      <TextInput
        ...
        value={num1}
        onChange={(e) => changeNum1(e.target.value)}
      />
      ...
      <TextInput
        ...
        style={[styles.containerInput]}
        value={num2}
        onChange={(e) => changeNum2(e.target.value)}
      />
      {/* second button */}
      <View style={[styles.button]}>
        <Button
          title={`Change from ` + name2()}
          style={[styles.button]}
          onPress={() => {
            // changes input value
            let len = dry ? itemsDry.length : itemsWet.length;
